...
const videos: VideoItem[] = [
    {
        id: "dQw4w9WgXcQ",
        title: "VNPT SmartBox phiên bản 2 – Biến TV thường thành thông minh",
        description: "Giải pháp truyền hình thông minh thế hệ mới với giao diện hiện đại, hỗ trợ 4K.",
        duration: "3:42",
    },
    {
        id: "ScMzIvxBSi4",
        title: "Hệ thống quản lý doanh nghiệp toàn diện ERP của ANSV",
        description: "Tối ưu hóa quy trình vận hành với giải pháp ERP tùy biến cho mọi ngành nghề.",
        duration: "5:18",
    },
    {
        id: "9bZkp7q19f0",
        title: "Chuyển đổi số – Cơ hội và thách thức cho doanh nghiệp Việt Nam",
        description: "Phân tích xu hướng chuyển đổi số và chiến lược triển khai hiệu quả.",
        duration: "7:05",
    },
    {
        id: "3tmd-ClpJxA",
        title: "Giải pháp An ninh mạng – Bảo vệ hạ tầng số doanh nghiệp",
        description: "Các giải pháp bảo mật toàn diện giúp doanh nghiệp an toàn trong kỷ nguyên số.",
        duration: "4:30",
    },
    {
        id: "kXYiU_JCYtU",
        title: "Big Data & AI – Khai thác sức mạnh dữ liệu lớn",
        description: "Ứng dụng trí tuệ nhân tạo và phân tích dữ liệu lớn trong môi trường doanh nghiệp.",
        duration: "6:12",
    },
    {
        id: "M7lc1UVf-VE",
        title: "Hạ tầng mạng thế hệ mới – Kết nối không giới hạn",
        description: "Thiết kế và triển khai hạ tầng mạng tốc độ cao, ổn định và bảo mật.",
        duration: "4:55",
    },
];
...
const VideoSection: React.FC = () => {
    ...
    const prev = () => switchTo((activeIdx - 1 + videos.length) % videos.length);
    const next = () => switchTo((activeIdx + 1) % videos.length);
    ...
    useEffect(() => {
        ...
        if (trackRef.current) {
            const el = trackRef.current.children[activeIdx] as HTMLElement;
            if (el) {
                // Chỉ scroll BÊN TRONG container, không đụng đến trang
                trackRef.current.scrollTop = el.offsetTop - trackRef.current.offsetTop - 12;
            }
        }
    }, [activeIdx]);
    ...
    return (
        <section className="video-section">
            ...
            <div className="section-inner" style={{ position: "relative", zIndex: 1 }}>
                {/* Header */}
                <div className="section-header">
                    <span className="section-label" style={{ color: "#e85d27" }}>NỘI DUNG NỔI BẬT</span>
                    <h2 className="section-title" style={{ color: "#fff" }}>Videos</h2>
                    <div className="video-header-line">
                        <span className="video-header-dot" />
                        <span className="video-header-bar" />
                        <span className="video-header-dot" />
                    </div>
                </div>
                ...
                <div className="video-main-layout">
                    ...
                    <div className="video-player-wrap">
                        <div className={`video-player-inner ${isTransitioning ? "v-fade-out" : "v-fade-in"}`}>
                            {playing ? (
                                <iframe
                                ...
                                <div className="video-thumbnail-wrap" onClick={() => setPlaying(true)}>
                                    <img
                                        ...
                                        onError={(e) => {
                                            (e.target as HTMLImageElement).src =
                                                `https://img.youtube.com/vi/${activeVideo.id}/hqdefault.jpg`;
                                        }}
                                    />
                                    ...
                                    <span className="video-duration-badge">{activeVideo.duration}</span>
                                </div>
                            )}
                        </div>

                        {/* Info below player */}
                        <div className={`video-info-bar ${isTransitioning ? "v-fade-out" : "v-fade-in"}`}>
                            <div className="video-info-index">
                                <span className="video-index-num">{String(activeIdx + 1).padStart(2, "0")}</span>
                                <span className="video-index-total">/ {String(videos.length).padStart(2, "0")}</span>
                            </div>
                            <div className="video-info-text">
                                <h3 className="video-featured-title">{activeVideo.title}</h3>
                                <p className="video-featured-desc">{activeVideo.description}</p>
                            </div>
                            <div className="video-nav-arrows">
                                <button className="video-arrow-btn" onClick={prev} aria-label="Previous">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18">
                                        <path d="M15 18l-6-6 6-6" />
                                    </svg>
                                </button>
                                <button className="video-arrow-btn" onClick={next} aria-label="Next">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18">
                    ...
                    <div className="video-playlist">
                        ...
                        <div className="video-playlist-track" ref={trackRef}>
                            {videos.map((v, i) => (
                                <div
                                    key={v.id}
                                    className={`video-playlist-item ${i === activeIdx ? "active" : ""}`}
                                    onClick={() => switchTo(i)}
                                >
                                    <div className="vpi-thumb">
                                        <img
                                            src={`https://img.youtube.com/vi/${v.id}/mqdefault.jpg`}
                                            alt={v.title}
                                            onError={(e) => {
                                                (e.target as HTMLImageElement).src =
                                                    `https://img.youtube.com/vi/${v.id}/default.jpg`;
                                            }}
                                        />
                                        {i === activeIdx && playing ? (
                                            <div className="vpi-playing-indicator">
                                                <span /><span /><span />
                                            </div>
                                        ) : (
                                            <div className="vpi-play-overlay">
                                                <svg viewBox="0 0 24 24" fill="currentColor" width="14" height="14">
                                                    <path d="M8 5v14l11-7z" />
                                                </svg>
                                            </div>
                                        )}
                                        <span className="vpi-duration">{v.duration}</span>
                                    </div>
                                    <div className="vpi-info">
                                        <span className="vpi-index">{String(i + 1).padStart(2, "0")}</span>
                                        <p className="vpi-title">{v.title}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="video-dots">
                    {videos.map((_, i) => (
                        <button
                            key={i}
                            className={`video-dot-btn ${i === activeIdx ? "active" : ""}`}
                            onClick={() => switchTo(i)}
